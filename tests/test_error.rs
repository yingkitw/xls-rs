...
fn test_error_with_file_context() {
    let error =
        XlsRsError::column_not_found("col").with_context(ErrorContext::new().with_file("data.csv"));
...
fn test_error_kind_code() {
    assert_eq!(
        ErrorKind::ColumnNotFound("x".to_string()).code(),
        "column_not_found"
    );
    assert_eq!(
        ErrorKind::InvalidCellRef("A1".to_string()).code(),
        "invalid_cell_ref"
    );
    assert_eq!(
        ErrorKind::InvalidValue("a".to_string(), "b".to_string()).code(),
        "invalid_value"
    );
    assert_eq!(
        ErrorKind::TypeConversion("a".to_string(), "b".to_string()).code(),
        "type_conversion"
    );
    assert_eq!(ErrorKind::DivisionByZero.code(), "division_by_zero");
    assert_eq!(
        ErrorKind::InvalidFormula("=SUM(".to_string()).code(),
        "invalid_formula"
    );
    assert_eq!(
        ErrorKind::FileNotFound("x".to_string()).code(),
        "file_not_found"
    );
    assert_eq!(
        ErrorKind::UnsupportedFormat("x".to_string()).code(),
        "unsupported_format"
    );
    assert_eq!(ErrorKind::ParseError("x".to_string()).code(), "parse_error");
    assert_eq!(
        ErrorKind::InvalidDateFormat("x".to_string()).code(),
        "invalid_date_format"
    );
    assert_eq!(
        ErrorKind::InvalidRegex("x".to_string()).code(),
        "invalid_regex"
    );
...
fn test_error_code_with_context() {
    let error = XlsRsError::column_not_found("price")
